...
if cmd == "check":
    ...
    r1=requests.post(url+"upload",files={'message':("data",open(fp,'rb'),'application/octet-stream')})
    ...
    if not message_id in r1.text:
        exit(102)
    r2 = requests.get(url+"run?message_id="+message_id+"&password="+password)
    if not flag in r2.text:
        exit(102)
    r = requests.get(url+"get?message_id="+message_id, stream=True)
