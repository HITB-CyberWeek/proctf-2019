...
def create_task(team, task_name, script_name, args, timeout=300):
    ...
    if retry_after > 0:
        retry_msg = "too fast, try again after %d secs" % retry_after
        return "429 Too fast", {"result": "too fast", "msg": retry_msg}
...
def cmd_get_vm_info(team, args):
    ...
    network_state = open("%s/team%d/network_state" % (DB_PATH, team)).read().strip()
    ...
    config = open("%s/team%d/serv%d_root_passwd.txt" % (DB_PATH, team, vm)).read().strip()
    return "200 Ok", {"result": "ok", "msg": "\n".join(msglines)}
...
def cmd_restore_vm_from_snapshot(team, args):
    ...
    if not re.fullmatch(r"[0-9a-zA-Z_]+", name):
        return "422 Failed to take snapshot", {"result": "bad snapshot name"}
    return create_task(team, "restore_vm_from_snapshot", "restore_vm_from_snapshot.py",
                       [str(team), str(vm), name])
...
def cmd_remove_snapshot(team, args):
    ...
    name = str(args[1])
    if not re.fullmatch(r"[0-9a-zA-Z_]+", name):
        return "422 Failed to remove snapshot", {"result": "bad vm name"}
...
def cmd_man(team, args):
  ...
  Step 2:
    Connect to vulnerable vm using ssh client:
      # get_vm_info <vm>
  Step 3:
    After initial setup, make your first vm snapshots, so you can recover to
...
def application(environ, start_response):
    ...
    if ("HTTP_X_AUTHORIZATION" not in environ or
            not environ["HTTP_X_AUTHORIZATION"].startswith("Bearier ")):
    ...
    CMDS = {
        ...
        "isolate_network": (cmd_isolate_network, 0, False, False),
        "open_network": (cmd_open_network, 0, False, False),
    ...
    if cmd not in CMDS:
        bad_cmd_msg = "bad command, type 'help' for help"
        start_response("422 Bad Request", RESP_HEADERS)
        return [json.dumps({"result": "error", "msg": bad_cmd_msg}).encode()]
    ...
    if first_arg_is_vm:
        ...
        if check_vm_created:
            image_deploy_state_file = "%s/team%d/serv%d_image_deploy_state" % (DB_PATH, team, vm)
            image_deploy_state = open(image_deploy_state_file).read().strip()
            if image_deploy_state != "RUNNING":
                start_response("422 Not yet", RESP_HEADERS)
                return [json.dumps({"result": "not yet", "msg": "vm is not created yet"}).encode()]
    ...
    log_msg = "cmd: team %d, %s -> %s" % (team, " ".join(map(str, [cmd] + args)), status)
