...
fun Intrinsic.runOn(argMap: Map<Variable, Any>, currentMachine: MachineState): MachineState {
    ...
    return when (this) {
        ...
        Intrinsic.WRITESTRING -> currentMachine.copy(output = currentMachine.output + String(argMap[ps[0]]!! as CharArray))
...
class NaiveInterpreter : Interpreter<MachineState, Statement, List<Int>> {
    ...
    fun Expression.evaluate(machine: MachineState): MachineState = when (this) {
        is IntLiteral -> machine.copy(result = value)
        is Variable ->
        ...
        is FunctionCall -> this.functionDeclaration.let { f ->
            ...
            for ((parameter, expr) in functionDeclaration.parameterNames.zip(argumentExpressions)) {
    ...
    override fun join(s: MachineState, p: Statement): MachineState = with(p) {
        when (this) {
            is Pass, is FunctionDeclaration -> s
            is AssignStatement -> expression.evaluate(s).run {
                if (shouldReturn)
                    this else
                    copy(state = state.plus(variable to result))
            }
            is IfStatement -> condition.evaluate(s).run {
                when {
                    shouldReturn -> this
            ...
            is ReturnStatement -> expression.evaluate(s).copy(shouldReturn = true)
